...
for d = 2
    ...
    if d==1
        ...
        path_to_mask = 'S:\MRI_Breast\data_train\NIfTI_Files_resaved_6';
        pat_dir = dir([path_to_dicom 'S*']);
        pat_dir = pat_dir(1:20);
    elseif d==2
        path_to_dicom = 'D:\Breast_MR_II\Export\DICOM\';
        path_to_mask = 'S:\MRI_Breast\data_train\NIfTI_Files_resaved_7';
        pat_dir = dir([path_to_dicom 'S*']);
    end
...
for pat = 17:length(pat_dir)
    ...
    maskGT = niftiread([path_to_mask filesep fName '_mask-labels.nii.gz'  ]);
    maskGT = flip(maskGT,2);
    maskGT = imresize3(maskGT,size(data),'nearest');
    
    %% splitting and prediction

    for verse = [0,1,2,5,6,7,8]
    % for verse = [8]

        % read model
        % net = load(['segm_3D_unet\trainedUNet_6_' num2str(verse) '.mat'],'net').net;
        net = load(['segm_3D_unet\trainedUNet_6_' num2str(verse) '.mat'],'netBest').netBest;
        ...
        for slice = 1+2:size(data,3)-2
            ...
            for i = 1:size(rects,1)
                ...
                mask_pred(rect(2):rect(2)+rect(4), rect(1):rect(1)+rect(3)) = max( mask_pred(rect(2):rect(2)+rect(4), rect(1):rect(1)+rect(3)) , pred);
            end
        ...
        % imshow5(lbl)
            ...
            if d==1
                path_save = ['S:\MRI_Breast\Data_predicted\BestNet_v6\MR_Breast_I\predicted_6_' num2str(verse) filesep ];
            elseif d==2
                path_save = ['S:\MRI_Breast\Data_predicted\BestNet_v6\MR_Breast_II\predicted_6_' num2str(verse) filesep ];
            end
        ...
        if verse==0
            ...
            niftiwrite(int16(maskGT),[path_save file_save '_maskGT'],nii_info,"Compressed",true)
        end
