...
imgdsVal = [imgds2(splitnum:end)];
maskdsVal = [maskds2(splitnum:end)];
...
name_version = 'trainedUNet_7_1';

description = ['instancni segm, all FNUSA data + bez DUKE, uceno from 4_7 (zmena posledni vrstvy) dataMining batch KCL, cyclic LR'];
...
net = load('trainedUNet_7_0.mat','net').net;

% lgraph = convolution2dLayer(3, 3, Padding="same", Name='encoderDecoderFinalConvLayer');
% net = removeLayers(net, 'encoderDecoderFinalConvLayer');
% net = addLayers(net, lgraph);
% net = connectLayers(net, 'Decoder-Stage-3-ReLU-2', 'encoderDecoderFinalConvLayer');
% net = connectLayers(net, 'encoderDecoderFinalConvLayer', 'FinalNetworkSoftmax-Layer');
...
monitor.Metrics = ["TrainingLoss","ValidationLoss","TrainingDiceBreast","ValidationDiceBreast","TrainingDiceImplants","ValidationDiceImplants"];
groupSubPlot(monitor,"Loss",["TrainingLoss","ValidationLoss"]);
groupSubPlot(monitor,"DiceBreast",["TrainingDiceBreast","ValidationDiceBreast"]);
groupSubPlot(monitor,"Diceimplants",["TrainingDiceImplants","ValidationDiceImplants"]);
...
while epoch < numEpochs && ~monitor.Stop
    ...
    while i < numIterationsPerEpoch && ~monitor.Stop
        ...
        % modelLoss function.
        [loss, gradients, ~, GDice] = dlfeval(@utils_net_train.modelLossDM,net,X,Toh);

        IDX(idx_Rand,3) = min( squeeze(extractdata(GDice)), [] , 1 );
        ...
        if mod(iteration, Val_iter) == 1
            ...
            for idxVal = 1:numel(imgdsVal)
                ...
                [vloss,~,vGDice] = dlfeval(@utils_net_train.modelLossDM,net,XVal,TVoh);
