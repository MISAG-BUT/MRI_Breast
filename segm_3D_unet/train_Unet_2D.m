...
% idxImplants = [idxImplants, 1, 2, 34,35,15,16];
...
name_version = 'trainedUNet_6_0';
% description = ['ucene z 5_0'];
description = ['input (4 channels) i s maskou breast - maska GT, segmentace jen implantatu'];
...
net = load('trainedUNet_6_0.mat','net').net;
...
learnRates = (linspace(1, 0.05 , numIterations)).^4;
...
Val_iter = round(numIterationsPerEpoch * (5));
...
while epoch < numEpochs && ~monitor.Stop
    ...
    while i < numIterationsPerEpoch && ~monitor.Stop
        ...
        % modelLoss function.
        [loss,gradients, GDice] = dlfeval(@utils_net_train.modelLoss,net,X,Toh);
